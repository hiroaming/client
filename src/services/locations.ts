...
export async function getCountriesWithPackages(): Promise<
...
> {
  ...
  packages.forEach((pkg) => {
    ...
    if (existing) {
      ...
      existing.minPriceUsd = Math.min(
        existing.minPriceUsd,
        pkg.price_usd_cents,
      );
...
export async function getRegionsWithPackages(): Promise<
...
export async function getPackagesForCountry(countryCode: string) {
  ...
  const { data, error } = await supabase
    ...
    .select("*")
    .eq("is_active", true)
    .containedBy("location_codes", [countryCode.toUpperCase()])
  ...
  console.log("Packages fetched for country:", countryCode, data);
